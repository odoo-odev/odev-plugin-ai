...
class SkillManager:
    ...
    def sync_skills(self, info: bool = False) -> None:
        ...
        disabled_names = self._get_disabled_skills()

        for tool, path_str in self.TOOL_PATHS.items():
            tool_skills_dir = Path(path_str).expanduser()
            if self._should_sync_tool(tool, tool_skills_dir):
                self._sync_tool_skills(tool, tool_skills_dir, available_skills, disabled_names, info)

    def _get_disabled_skills(self) -> list[str]:
        """Retrieve disabled skill names from config."""
        try:
            return self.odev.config.skills.disabled
        except AttributeError:
            return []

    def _should_sync_tool(self, tool: str, tool_skills_dir: Path) -> bool:
        """Determine if a tool's skills should be synchronized."""
    ...
    def _sync_tool_skills(
        ...
        """Perform synchronization for a specific tool."""
